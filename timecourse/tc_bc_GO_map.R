...
### detected protein only background ---------------------
...
# go_40 <- read.csv("data/timecourse/botrytis/GO/tc_bc_GO_BPsig_hpi_40_revigo.csv") %>%
# 	pull(Term)
all_go <- sort(unique(c(go_16, go_24)))

#create empty vectors for 8, 32,  and 48
go_8 <- c()
go_32 <- c()
go_40 <- c()
go_48 <- c()
...
ggplot(go_long, aes(x = Timepoint, y = GO, fill = Present)) +
	...
	scale_y_discrete(limits = rev) +
	ylab("") +
	xlab("First Detection Time\n(HPI)")

ggsave("figures/timecourse/botrytis/tc_bc_GO_map.png", height = 4, width = 4, dpi = 2000)
