...
##### For modeling Botrytis protein separately at each timepoint
##### March 2026 AJM
...
analyze_protein <- function(protein_ID, bc) {
  ...
  hpi_vals <- unique(as.character(bc$hpi))
  ...
  for (hpi_val in hpi_vals) {
    ...
    bc_hpi <- bc %>% filter(as.character(hpi) == hpi_val)
    ...
    model <- glmmTMB(formula, data = bc_hpi, family = nbinom2) %>%
      suppressMessages()
